...
An interactive, pixelated 3D globe that visualises the percentage of the tech workforce that is female in countries around the world. The project highlights the global gender gap in technology through a beautiful pink-themed, retro-pixel aesthetic.

### Features

- **3D Rotating Globe** — A low-poly, flat-shaded icosahedron globe that auto-rotates and can be dragged to explore
- **57 Country Data Points** — Pixelated box columns rise from each country, with height and colour mapped to female tech participation rates
...
- **Pink Aesthetic** — Entire colour palette in shades of pink, magenta, and rose — from the deep dark background to glowing markers and star field
...
Each country is represented by a `BoxGeometry` column positioned on the globe surface using lat/lon → spherical coordinate conversion. Column height and colour both encode the female tech workforce percentage, creating a gradient from deep magenta (low %) to bright pink (high %).
